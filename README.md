...
- [x] **M12 — Update check**: the lightweight nudge M10 deferred — a background thread checks GitHub's latest release tag against the running version on launch, and an "Update to vX.Y.Z" button appears in the header only if one's actually newer. stdlib `urllib`, no new dependency; degrades to silence (never a crash) offline, rate-limited, or before the repo even existed — verified by launching the app against a 404
